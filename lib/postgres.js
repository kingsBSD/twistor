...
module.exports = {
	...
	select: args => {
		const q = `SELECT * FROM users,tweets,deletions WHERE users.id = tweets.user_id AND deletions.id = tweets.id) ${args.u ? "AND users.id = $3 " : ""}ORDER BY ${_.contains(sorts, args.sort) ? _.find(sorts, val => val == args.sort) : "delete_time"} ${args.asc ? "ASC" : "DESC"} LIMIT $1 OFFSET $2`;
		console.log(q);

		//so it doesn't matter what order they come in
		const params = [].concat(
			args.take > 0 && args.take <= 50 && args.take == (args.take|0) ? args.take : 20,
